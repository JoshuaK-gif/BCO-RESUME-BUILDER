...
export const Graduate3Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins}mm` }}>
      ...
      {summary && <section style={{ marginBottom: '20px', backgroundColor: '#f0fdf4', padding: '16px', borderRadius: '8px' }}><p style={{ whiteSpace: 'pre-line', color: '#374151', margin: 0 }}>{summary}</p></section>}
      ...
      {experience.length > 0 && (
        <section style={{ marginBottom: '20px' }}>
          <h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 800, color: design.accentColor, marginBottom: '10px' }}>Experience</h2>
          {experience.map((exp) => (
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {skills.map((g) => g.skills.map((s, i) => (
              <span key={`${g.id}-${i}`} style={{ padding: '4px 10px', backgroundColor: '#dcfce7', color: '#166534', fontSize: `${design.fontSize - 1}px`, fontWeight: 600, borderRadius: '12px' }}>{s}</span>
            )))}
          </div>
        </section>
      )}
