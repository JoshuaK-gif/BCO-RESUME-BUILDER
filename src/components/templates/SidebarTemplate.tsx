...
export const SidebarTemplate: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  const { personalInfo, summary, experience, education, skills, projects, certifications, languages } = content;

  const SidebarHeading = ({ title, icon }: { title: string; icon?: string }) => (
    <h3 style={{
      fontSize: `${design.fontSize - 2}px`, fontWeight: 700, color: design.accentColor,
      textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '10px',
      paddingBottom: '6px', borderBottom: '1px solid rgba(255,255,255,0.2)',
  ...
  const MainHeading = ({ title, icon }: { title: string; icon?: string }) => (
    <div style={{
      ...
      <h2 style={{
        ...
        borderBottom: `2px solid ${design.accentColor}20`, flex: 1,
      }}>{title}</h2>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      color: '#334155', backgroundColor: '#fff', minHeight: '297mm', display: 'flex',
    }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', bottom: '80px', left: '-20px', width: '60px', height: '60px', borderRadius: '50%', background: `${design.accentColor}10` }} />
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{
            ...
            background: `linear-gradient(135deg, ${design.accentColor}, ${design.accentColor}cc)`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            margin: '0 auto 14px', fontSize: '26px', fontWeight: 700, color: '#fff',
          ...
          <div style={{ textAlign: 'center', marginBottom: '20px' }}>
            <h1 style={{ fontSize: `${design.headingSize + 2}px`, fontWeight: 700, margin: 0, color: '#fff', lineHeight: 1.2 }}>
              {personalInfo.fullName || 'Your Name'}
            </h1>
            {personalInfo.professionalTitle && (
              <p style={{ color: design.accentColor, marginTop: '4px', fontSize: `${design.fontSize - 1}px` }}>
                {personalInfo.professionalTitle}
              </p>
            )}
          </div>
          ...
          {/* Contact */}
          <SidebarHeading title="Contact" icon="✉" />
          ...
          {skills.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              <SidebarHeading title="Skills" icon="⚡" />
              {skills.map((g) => (
                <div key={g.id} style={{ marginBottom: '12px' }}>
                  <div style={{ fontWeight: 600, fontSize: `${design.fontSize - 1.5}px`, marginBottom: '6px', color: design.accentColor }}>{g.category}</div>
                  {g.skills.map((s, i) => (
          ...
          {education.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              <SidebarHeading title="Education" icon="🎓" />
              {education.map((edu) => (
          ...
          {certifications.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              <SidebarHeading title="Certifications" icon="🏆" />
              {certifications.map((c) => (
                <div key={c.id} style={{ fontSize: `${design.fontSize - 1.5}px`, marginBottom: '6px' }}>
          ...
          {languages.length > 0 && (
            <div>
              <SidebarHeading title="Languages" icon="🌍" />
              {languages.map((l) => (
                <div key={l.id} style={{ fontSize: `${design.fontSize - 1}px`, marginBottom: '4px', opacity: 0.85 }}>
                  <span style={{ width: '6px', height: '6px', background: design.accentColor, borderRadius: '50%', display: 'inline-block', marginRight: '6px' }} />
      ...
      <div style={{ flex: 1, padding: `${design.margins}mm` }}>
        {summary && (
          <section style={{ marginBottom: `${design.sectionSpacing}px` }}>
            <MainHeading title="Professional Summary" icon="★" />
            <div style={{
        ...
        {experience.length > 0 && (
          <section style={{ marginBottom: `${design.sectionSpacing}px` }}>
            <MainHeading title="Experience" icon="💼" />
            {experience.map((exp, idx) => (
              <div key={exp.id} style={{
                ...
                <p style={{ color: design.accentColor, fontWeight: 600, margin: '2px 0 6px', fontSize: `${design.fontSize - 0.5}px` }}>
        ...
        {projects.length > 0 && (
          <section>
            <MainHeading title="Projects" icon="🚀" />
            {projects.map((p) => (
