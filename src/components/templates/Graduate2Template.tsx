...
export const Graduate2Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins}mm` }}>
      ...
      {summary && <section style={{ marginBottom: '16px' }}><h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 700, color: design.accentColor, marginBottom: '8px' }}>Objective</h2><p style={{ whiteSpace: 'pre-line', color: '#374151' }}>{summary}</p></section>}
        ...
        <section style={{ marginBottom: '16px' }}>
          ...
          {education.map((edu) => (
            <div key={edu.id} style={{ marginBottom: '8px' }}>
              ...
              {edu.gpa && <p style={{ fontSize: `${design.fontSize - 1}px`, color: '#666' }}>GPA: {edu.gpa}</p>}
            </div>
          ))}
        </section>
      )}
