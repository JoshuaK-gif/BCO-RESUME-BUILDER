...
export const Professional12Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {skills.map((g) => g.skills.map((s, i) => (
              <span key={`${g.id}-${i}`} style={{ padding: '3px 10px', backgroundColor: `${design.accentColor}10`, borderRadius: '3px', fontSize: `${design.fontSize - 1}px` }}>{s}</span>
            )))}
          </div>
        </section>
      )}
