...
export const Clean2Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins * 1.5}mm` }}>
      <h1 style={{ fontSize: `${design.headingSize + 14}px`, fontWeight: 600, margin: 0 }}>{personalInfo.fullName || 'Your Name'}</h1>
      ...
      {experience.length > 0 && (
        <section style={{ marginBottom: '16px' }}>
          <h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 600, color: '#111', marginBottom: '10px' }}>Experience</h2>
          {experience.map((exp) => (
      ...
      {skills.length > 0 && (
        <section>
          <h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 600, color: '#111', marginBottom: '8px' }}>Skills</h2>
