...
export const Professional6Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins}mm` }}>
      <div style={{ borderBottom: `3px double ${design.accentColor}`, paddingBottom: '16px', marginBottom: '20px' }}>
        <h1 style={{ fontSize: `${design.headingSize + 14}px`, fontWeight: 700, margin: 0, textAlign: 'center' }}>{personalInfo.fullName || 'Your Name'}</h1>
        {personalInfo.professionalTitle && <p style={{ fontSize: `${design.fontSize + 2}px`, color: '#666', marginTop: '4px', textAlign: 'center' }}>{personalInfo.professionalTitle}</p>}
