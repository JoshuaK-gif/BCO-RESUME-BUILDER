...
export const Executive5Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  const { personalInfo, summary, experience, education, skills, projects, certifications, languages } = content;

  const SectionTitle = ({ title, icon }: { title: string; icon: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px', paddingBottom: '8px', borderBottom: `3px solid ${design.accentColor}` }}>
      <div style={{ width: '30px', height: '30px', borderRadius: '6px', background: design.accentColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', color: '#fff', fontWeight: 700, flexShrink: 0 }}>{icon}</div>
      <h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 900, color: '#0f172a', margin: 0, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{title}</h2>
    </div>
  );
    ...
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1e293b', backgroundColor: '#fff', minHeight: '297mm' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '20px', alignItems: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{ fontSize: `${design.nameSize + 4}px`, fontWeight: 900, margin: 0, letterSpacing: '-0.01em', lineHeight: 1.1, textTransform: 'uppercase' }}>
              {personalInfo.fullName || 'Your Name'}
            </h1>
            {personalInfo.professionalTitle && <p style={{ fontSize: `${design.fontSize + 3}px`, opacity: 0.9, marginTop: '4px', fontWeight: 300, letterSpacing: '0.06em' }}>{personalInfo.professionalTitle}</p>}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '16px' }}>
              {personalInfo.email && <span style={{ fontSize: `${design.fontSize - 1}px`, background: 'rgba(255,255,255,0.15)', padding: '4px 12px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.2)' }}>✉ {personalInfo.email}</span>}
              {personalInfo.phone && <span style={{ fontSize: `${design.fontSize - 1}px`, background: 'rgba(255,255,255,0.15)', padding: '4px 12px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.2)' }}>☎ {personalInfo.phone}</span>}
              {personalInfo.location && <span style={{ fontSize: `${design.fontSize - 1}px`, background: 'rgba(255,255,255,0.15)', padding: '4px 12px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.2)' }}>📍 {personalInfo.location}</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: `${design.margins}mm` }}>
        {summary && (
          <section style={{ marginBottom: `${design.sectionSpacing + 4}px` }}>
            ...
            <div style={{ padding: '12px 16px', background: '#f8fafc', borderRadius: '8px', borderLeft: `4px solid ${design.accentColor}`, color: '#475569', whiteSpace: 'pre-line', fontSize: `${design.fontSize}px` }}>{summary}</div>
          </section>
        )}
        ...
        {experience.length > 0 && (
          <section style={{ marginBottom: `${design.sectionSpacing + 4}px` }}>
            ...
            {experience.map((exp, idx) => (
              <div key={exp.id} style={{ marginBottom: idx < experience.length - 1 ? '18px' : '0', paddingBottom: idx < experience.length - 1 ? '18px' : '0', borderBottom: idx < experience.length - 1 ? '1px solid #e2e8f0' : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <h3 style={{ fontSize: `${design.headingSize}px`, fontWeight: 700, margin: 0, color: '#0f172a' }}>{exp.position}</h3>
                    <p style={{ color: design.accentColor, fontWeight: 600, fontSize: `${design.fontSize}px`, margin: '2px 0 6px' }}>{exp.company}{exp.location ? ` · ${exp.location}` : ''}</p>
                  </div>
                  <span style={{ fontSize: `${design.fontSize - 1}px`, color: '#64748b', padding: '3px 10px', background: `${design.accentColor}10`, borderRadius: '12px', fontWeight: 500, whiteSpace: 'nowrap', marginLeft: '12px' }}>
          ...
          <section style={{ marginBottom: `${design.sectionSpacing + 4}px` }}>
            ...
            {education.map((edu) => (
              <div key={edu.id} style={{ marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: `${design.fontSize - 1}px`, color: '#94a3b8', padding: '3px 10px', background: `${design.accentColor}08`, borderRadius: '12px' }}>{edu.startDate} – {edu.endDate}</span>
              </div>
            ))}
          </section>
        )}
