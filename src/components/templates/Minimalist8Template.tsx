...
export const Minimalist8Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins * 1.5}mm` }}>
      ...
      {summary && <section style={{ marginBottom: '20px' }}><p style={{ whiteSpace: 'pre-line', color: '#444' }}>{summary}</p></section>}

      {experience.length > 0 && (
        <section style={{ marginBottom: '20px' }}>
          <h2 style={{ fontSize: `${design.fontSize - 1}px`, fontWeight: 400, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '10px', color: '#888' }}>Experience</h2>
          {experience.map((exp) => (
      ...
      {education.length > 0 && (
        <section style={{ marginBottom: '20px' }}>
          <h2 style={{ fontSize: `${design.fontSize - 1}px`, fontWeight: 400, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '10px', color: '#888' }}>Education</h2>
          {education.map((edu) => (
      ...
      {skills.length > 0 && (
        <section>
          <h2 style={{ fontSize: `${design.fontSize - 1}px`, fontWeight: 400, textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '10px', color: '#888' }}>Skills</h2>
