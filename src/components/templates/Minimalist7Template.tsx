...
export const Minimalist7Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins * 1.5}mm` }}>
      <h1 style={{ fontSize: `${design.headingSize + 16}px`, fontWeight: 300, margin: 0, letterSpacing: '-0.01em' }}>{personalInfo.fullName || 'Your Name'}</h1>
      <div style={{ marginTop: '6px', fontSize: `${design.fontSize - 1}px`, color: '#888' }}>
      ...
      {experience.length > 0 && (
        <section style={{ marginBottom: '20px' }}>
          <h2 style={{ fontSize: `${design.fontSize}px`, fontWeight: 400, marginBottom: '10px', color: '#111' }}>Experience</h2>
          {experience.map((exp) => (
      ...
      {education.length > 0 && (
        <section style={{ marginBottom: '20px' }}>
          <h2 style={{ fontSize: `${design.fontSize}px`, fontWeight: 400, marginBottom: '10px', color: '#111' }}>Education</h2>
          {education.map((edu) => (
      ...
      {skills.length > 0 && (
        <section>
          <h2 style={{ fontSize: `${design.fontSize}px`, fontWeight: 400, marginBottom: '10px', color: '#111' }}>Skills</h2>
