...
export const ATS3Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#000', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins}mm` }}>
      ...
      {summary && <section style={{ marginTop: '16px', marginBottom: '12px' }}><h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 700, marginBottom: '6px' }}>Professional Summary</h2><p style={{ whiteSpace: 'pre-line' }}>{summary}</p></section>}

      {experience.length > 0 && (
        <section style={{ marginBottom: '12px' }}>
          <h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 700, marginBottom: '8px' }}>Work Experience</h2>
          {experience.map((exp) => (
      ...
      {skills.length > 0 && (
        <section>
          <h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 700, marginBottom: '6px' }}>Key Skills</h2>
          {skills.map((g) => (
