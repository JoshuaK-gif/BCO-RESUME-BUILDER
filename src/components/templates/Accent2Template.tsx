...
export const Accent2Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins}mm` }}>
      <div style={{ display: 'flex', gap: '30px' }}>
        <div style={{ width: '4px', backgroundColor: design.accentColor, borderRadius: '2px' }} />
        <div>
      ...
      {summary && <section style={{ marginTop: '20px', marginBottom: '16px' }}><h2 style={{ fontSize: `${design.headingSize}px`, fontWeight: 700, color: design.accentColor, marginBottom: '8px' }}>Profile</h2><p style={{ whiteSpace: 'pre-line', color: '#374151' }}>{summary}</p></section>}
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {skills.map((g) => g.skills.map((s, i) => (
              <span key={`${g.id}-${i}`} style={{ padding: '3px 10px', borderLeft: `3px solid ${design.accentColor}`, backgroundColor: `${design.accentColor}10`, fontSize: `${design.fontSize - 1}px` }}>{s}</span>
            )))}
          </div>
        </section>
      )}
