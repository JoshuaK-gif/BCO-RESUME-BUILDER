...
export const Industry2Template: React.FC<{ content: ResumeContent; design: DesignSettings }> = ({ content, design }) => {
  ...
  return (
    <div style={{ fontFamily: design.font, fontSize: `${design.fontSize}px`, lineHeight: design.lineHeight, color: '#1a1a1a', backgroundColor: '#fff', minHeight: '297mm', padding: `${design.margins}mm` }}>
      <div style={{ backgroundColor: design.accentColor, color: '#fff', padding: '28px', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', marginTop: '12px', fontSize: `${design.fontSize - 1}px` }}>
          {personalInfo.email && <span style={{ padding: '4px 12px', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '4px' }}>{personalInfo.email}</span>}
          {personalInfo.phone && <span style={{ padding: '4px 12px', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '4px' }}>{personalInfo.phone}</span>}
          {personalInfo.location && <span style={{ padding: '4px 12px', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '4px' }}>{personalInfo.location}</span>}
        </div>
      </div>
      ...
      {experience.length > 0 && (
        <section style={{ marginBottom: '20px' }}>
          ...
          {experience.map((exp) => (
            <div key={exp.id} style={{ marginBottom: '16px', padding: '16px', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
              ...
              <p style={{ color: design.accentColor, fontWeight: 700, marginTop: '4px' }}>{exp.company}</p>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {skills.map((g) => g.skills.map((s, i) => (
              <span key={`${g.id}-${i}`} style={{ padding: '6px 14px', backgroundColor: design.accentColor, color: '#fff', fontSize: `${design.fontSize - 1}px`, fontWeight: 700, borderRadius: '6px' }}>{s}</span>
            )))}
          </div>
        </section>
      )}
